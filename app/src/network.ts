...
import { isByeMsg, isConnectedMsg, isWelcomeMsg, isXCommand } from "./protocol";
...
type Fn<P, R> = (x: P) => R;
const not = <T>(f: Fn<T, boolean>) => (x: T) => !f(x);
...
export const isGoFullscreenCommand = isXCommand<GoFullscreenCommand>(
  "go-full-screen",
);
...
export const fullscreen = socket.pipe(
  filter(isGoFullscreenCommand),
  pluck("payload"),
);

export const sendAsType =
  <T extends CommandTypes>(type: T) => (payload: PayloadOfType<T>) =>
